...
def resolve_workspace_path(
    ...
    *,
    ...
    if relative_path.startswith("/"):
        raise PathContainmentError(f"absolute paths are not allowed: {relative_path}")

    # Reject .. traversal
    parts = Path(relative_path).parts
    if ".." in parts:
        raise PathContainmentError(f"path traversal is not allowed: {relative_path}")
    ...
    if not str(target).startswith(str(root)):
        raise PathContainmentError(f"path escapes the workspace: {relative_path} -> {target}")

    # Reject symlinks (unless explicitly allowed)
    if not follow_symlinks and target.is_symlink():
        raise PathContainmentError(f"symlinks are not allowed: {relative_path}")

    # Check existence if required
    if must_exist and not target.exists():
        raise PathContainmentError(f"file does not exist: {relative_path}")
