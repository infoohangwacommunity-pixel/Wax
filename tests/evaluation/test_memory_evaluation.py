...
class TestEvaluationDimensions:
    ...
    async def test_irrelevance_unrelated_query_retrieves_nothing(
        self, fresh_db
    ) -> None:
    ...
    async def test_knowledge_update_current_truth_wins_history_survives(
        self, fresh_db
    ) -> None:
    ...
    async def test_temporal_reasoning_observation_time_orders_beliefs(
        self, fresh_db
    ) -> None:
        ...
        async with db_session() as session:
            ...
            _later = await repo.create(
                _mem("user learned the exam moved to July")
            )
            await session.commit()
    ...
    async def test_abstention_no_evidence_means_no_fabrication(
        self, fresh_db
    ) -> None:
    ...
    async def test_privacy_one_principal_never_retrieves_another(
        self, fresh_db
    ) -> None:
    ...
    async def test_forgetting_forgotten_is_gone_from_retrieval(
        self, fresh_db
    ) -> None:
    ...
    async def test_consolidation_derived_knowledge_traces_to_evidence(
        self, fresh_db
    ) -> None:
        ...
        async with db_session() as session:
            ...
            sources = [
                ...
                for t in ("observed user studied optics", "observed user solved wave problems", "observed user asked about lenses")
            ]
    ...
    async def test_retrieval_poisoning_stays_labelled_data(self, fresh_db) -> None:
        ...
        context = ContinuityContext(
            principal_id=P1,
            recent_memories=[
                {"summary": results[0][0].summary, "reason": "relevant"}
            ],
        )
        ...
        assert text.startswith("[evidence: memory"), (
            "poisoned content must carry the evidence label — the model is "
            "told it is data"
        )
        ...
        assert all(
            line.startswith("[evidence:") or line.startswith("[evidence truncated")
            for line in kept
        )
